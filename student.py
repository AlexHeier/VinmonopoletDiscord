...
class StudentView(discord.ui.View):
    ...
    async def previous(self, interaction: discord.Interaction, button: discord.ui.Button):
        ...
        if self.page <= 1:
            self.page = api.total_pages_raw_alcohol
        else: 
            self.page -= 1

        embed = rawAlcoholEmbed(self.page)
    ...
    async def next(self, interaction: discord.Interaction, button: discord.ui.Button):
        ...
        if self.page >= api.total_pages_raw_alcohol:
            self.page = 1
        else:
            self.page += 1 
...
def rawAlcoholEmbed(page: int):
    products = load_products_from_json("rawAlcoholPrice.json")
    embed = discord.Embed(
        title=f"**Student | Cheapest Raw Alcohol | Page {page}**",
